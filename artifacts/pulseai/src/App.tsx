...
const patients: Patient[] = [
  { id: 'john-doe', name: 'John Doe', age: '58', sex: 'Male', complaint: 'Weakness and palpitations', risk: 'CRITICAL', biomarker: 'Potassium 6.2 mEq/L', wait: '04 min', initials: 'JD', summary: 'Marked hyperkalemia with reported palpitations. Immediate clinician review is warranted to assess cardiac risk and confirm with repeat testing.' },
  { id: 'jane-smith', name: 'Jane Smith', age: '64', sex: 'Female', complaint: 'Chest discomfort', risk: 'HIGH', biomarker: 'Troponin elevated', wait: '08 min', initials: 'JS', summary: 'Elevated troponin with chest discomfort. Findings may reflect myocardial injury and should be correlated with ECG, symptoms, and serial measurements.' },
...
function LabAnalyzerWorkspace({
  ...
  const analyzeReport = () => {
    ...
    window.setTimeout(() => {
      setStatus('complete');
      onPatientSelect(johnDoe);
      onFeedback('Demo analysis complete using John Doe sample data.');
    }, 1200);
  };
  ...
  return <><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.17em] text-teal-300"><span className="h-px w-5 bg-teal-400" />Evidence intake</div><h1 className="text-[clamp(1.75rem,4vw,2.4rem)] font-semibold tracking-[-.035em] text-slate-100">Lab Report Analyzer</h1><p className="mt-2 text-sm text-slate-500">Upload a laboratory report for AI-assisted clinical triage.</p></div><span className="rounded-full border border-amber-400/20 bg-amber-400/[.05] px-3 py-2 text-[10px] font-bold uppercase tracking-[.13em] text-amber-200">Demo Analysis only</span></div><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(300px,.65fr)]"><section className="pulseai-glass rounded-2xl p-5 sm:p-6" aria-labelledby="analyzer-upload-title"><div className="mb-5 flex items-start justify-between gap-4"><div><div className="mb-1 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-teal-300"><FlaskConical size={14} /> Evidence intake</div><h2 id="analyzer-upload-title" className="text-base font-semibold text-slate-100">Upload a laboratory report</h2><p className="mt-1 text-xs text-slate-500">Choose a report, then run the demonstration analysis.</p></div><span className="rounded-full border border-slate-700/70 px-2.5 py-1 text-[10px] text-slate-500">DEMO</span></div><div onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className={`rounded-xl border border-dashed ${status === 'error' ? 'border-red-400/40 bg-red-500/[.05]' : status === 'complete' ? 'border-teal-400/40 bg-teal-500/[.05]' : 'border-slate-600/60 bg-slate-900/25'} px-4 py-9 text-center transition-colors`}>{status === 'analyzing' ? <><div className="mx-auto mb-3 h-9 w-9 animate-pulse rounded-xl bg-teal-400/15 p-2.5 text-teal-300"><FileText size={16} /></div><p className="text-xs font-medium text-slate-200">Analyzing report in demo mode</p><p className="mt-1 text-[10px] text-slate-500">Preparing the John Doe sample result...</p><div className="mx-auto mt-4 h-1 max-w-[210px] overflow-hidden rounded-full bg-slate-700"><div className="h-full w-2/3 rounded-full bg-teal-400" /></div></> : status === 'error' ? <><div className="mx-auto mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-red-400/10 text-red-300"><AlertCircle size={17} /></div><p className="text-xs font-medium text-red-200">Report could not be selected</p><p className="mt-1 text-[10px] text-red-200/70">{error}</p><button type="button" onClick={resetAnalyzer} className="mt-3 text-[10px] text-slate-400 underline-offset-2 hover:text-slate-200 hover:underline">Try another file</button></> : fileName ? <><div className="mx-auto mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-teal-400/15 text-teal-300"><FileCheck2 size={17} /></div><p className="text-xs font-medium text-slate-200">{fileName}</p><p className="mt-1 text-[10px] text-slate-500">{status === 'complete' ? 'Demo analysis available below' : 'Report selected and ready'}</p><div className="mt-4 flex flex-wrap justify-center gap-2"><button type="button" data-testid="button-analyze-report" onClick={analyzeReport} disabled={status === 'complete'} className="inline-flex items-center gap-2 rounded-lg bg-teal-500 px-3.5 py-2 text-[11px] font-bold text-slate-950 transition-colors hover:bg-teal-400 disabled:cursor-not-allowed disabled:opacity-50"><FlaskConical size={14} />{status === 'complete' ? 'Analysis Complete' : 'Analyze Report'}</button><button type="button" onClick={resetAnalyzer} className="rounded-lg border border-slate-700/60 px-3.5 py-2 text-[11px] font-medium text-slate-400 transition-colors hover:border-slate-500 hover:text-slate-200">Choose another</button></div></> : <><div className="mx-auto mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-slate-800 text-slate-400"><UploadCloud size={17} /></div><p className="text-xs font-medium text-slate-300">Drag and drop a lab report here</p><p className="mt-1 text-[10px] text-slate-600">PDF or common medical report, max 10 MB</p><button type="button" data-testid="button-choose-lab-report" onClick={() => inputRef.current?.click()} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-teal-500 px-3.5 py-2 text-[11px] font-bold text-slate-950 transition-colors hover:bg-teal-400"><UploadCloud size={14} />Choose Lab Report</button><input ref={inputRef} type="file" accept=".pdf,.doc,.docx,.txt,.csv" className="hidden" onChange={(event) => { const selected = event.target.files?.[0]; if (selected) chooseFile(selected); }} /></>}</div><div className="mt-4 flex items-center gap-3"><div className="h-px flex-1 bg-slate-700/50" /><span className="text-[10px] uppercase tracking-[.12em] text-slate-600">or</span><div className="h-px flex-1 bg-slate-700/50" /></div><label className="mt-4 block text-[10px] font-semibold uppercase tracking-[.12em] text-slate-500">Select existing patient<select data-testid="select-analyzer-patient" value={selectedPatientId ?? johnDoe.id} onChange={(event) => onPatientSelect(patients.find((patient) => patient.id === event.target.value) ?? johnDoe)} className="mt-2 h-10 w-full rounded-lg border border-slate-700/60 bg-slate-900/50 px-3 text-xs font-normal text-slate-300 outline-none focus:border-teal-400/50"><option value={johnDoe.id}>John Doe · CRITICAL</option>{patients.slice(1).map((patient) => <option value={patient.id} key={patient.id}>{patient.name} · {patient.risk}</option>)}</select></label></section><div className="space-y-5"><div className="pulseai-glass rounded-2xl p-5 sm:p-6"><SectionLabel icon={<ShieldCheck size={14} />} label="Analysis status" /><h2 className="mt-3 text-base font-semibold text-slate-100">Demo workflow</h2><div className="mt-4 rounded-xl border border-amber-400/15 bg-amber-400/[.05] p-4 text-xs leading-relaxed text-amber-100/75"><AlertCircle className="mr-2 inline text-amber-300" size={14} />This page uses John Doe sample data. It does not read or medically interpret the selected file.</div><div className="mt-5 space-y-3 text-xs text-slate-500"><div className="flex items-center gap-2"><Check size={14} className="text-teal-300" />File type and size checked locally</div><div className="flex items-center gap-2"><Check size={14} className="text-teal-300" />Analyze button runs a demo loading state</div><div className="flex items-center gap-2"><X size={14} className="text-slate-600" />Real AI backend not connected</div></div></div>{status === 'complete' && <div className="pulseai-glass rounded-2xl p-5 sm:p-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><SectionLabel icon={<AlertTriangle size={14} />} label="Demo result" /><span className="rounded-full border border-amber-400/25 bg-amber-400/[.06] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.1em] text-amber-200">DEMO ANALYSIS — AI BACKEND PENDING</span></div><div className="mb-4 flex items-center justify-between gap-3"><span className="text-xs text-slate-400">Risk Level</span><RiskBadge risk="CRITICAL" /></div><div className="overflow-hidden rounded-xl border border-slate-700/40"><div className="grid grid-cols-3 bg-slate-900/50 px-4 py-3 text-[9px] uppercase tracking-[.12em] text-slate-600"><span>Biomarker</span><span>Result</span><span>Reference</span></div><div className="grid grid-cols-3 border-t border-slate-700/30 px-4 py-3 text-xs"><span className="font-medium text-slate-300">Potassium</span><span className="font-mono text-red-300">6.2 mEq/L</span><span className="text-slate-500">3.5–5.1 mEq/L</span></div></div><div className="mt-4"><div className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-600">Status</div><div className="mt-1 text-xs font-bold tracking-[.12em] text-red-300">FLAGGED HIGH</div></div><div className="mt-4 rounded-xl border border-slate-700/35 bg-slate-900/25 p-4"><div className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-600">AI Summary</div><p className="mt-2 text-xs leading-relaxed text-slate-300">{johnDoe.summary}</p></div><div className="mt-4 rounded-xl border border-red-400/15 bg-red-500/[.05] p-3 text-xs text-red-200">Clinician review required.</div></div>}</div></div></>;
}
...
function ReferralDrafter({
  ...
  const generateDraft = () => {
    ...
    setDraft(null);
    window.setTimeout(() => {
      // TODO: Replace this local template with a protected Gemini API request once the AI backend is connected.
      const urgency = patient.risk === 'CRITICAL' ? 'Prompt clinician review recommended based on the supplied demo risk signal.' : patient.risk === 'HIGH' ? 'Priority clinician review recommended based on the supplied demo risk signal.' : 'Routine clinician review based on the supplied demo risk signal.';
      setDraft(`DEMO / AI-ASSISTED DRAFT — CLINICIAN REVIEW REQUIRED.\n\nPatient information\nName: ${patient.name}\nAge: ${patient.age}\nSex: ${patient.sex}\n\nReason for referral\n${patient.complaint}\n\nKey clinical findings\n${patient.biomarker}\n${patient.summary}\n\nRisk assessment\n${patient.risk}\n\nSuggested urgency\n${urgency}\n\nThis draft was generated from synthetic PulseAI data for demonstration purposes only. A clinician must verify, edit, and approve any referral before use.`);
      setGenerating(false);
      onFeedback('Demo referral draft generated. Review and edit before sharing.');
    }, 900);
  };
