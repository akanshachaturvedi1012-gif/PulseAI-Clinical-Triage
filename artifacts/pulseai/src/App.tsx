...
import { Activity, AlertCircle, AlertTriangle, ArrowRight, Bell, Check, ChevronDown, ClipboardList, Clock3, FileCheck2, FileText, FlaskConical, HeartPulse, LayoutDashboard, Menu, MessageSquareText, PanelLeftClose, Search, Settings, ShieldCheck, Stethoscope, UploadCloud, UserRound, UsersRound, X, Zap } from 'lucide-react';
...
function LabAnalyzerWorkspace({
  ...
  return <><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.17em] text-teal-300"><span className="h-px w-5 bg-teal-400" />Evidence intake</div><h1 className="text-[clamp(1.75rem,4vw,2.4rem)] font-semibold tracking-[-.035em] text-slate-100">Lab Report Analyzer</h1><p className="mt-2 text-sm text-slate-500">Prepare a report for a demonstration workflow — clinical analysis is not connected.</p></div><span className="rounded-full border border-amber-400/20 bg-amber-400/[.05] px-3 py-2 text-[10px] font-bold uppercase tracking-[.13em] text-amber-200">Demo Analysis only</span></div><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(300px,.65fr)]"><UploadPanel title="Upload a laboratory report" selectedPatientId={selectedPatientId} onPatientSelect={onPatientSelect} onFeedback={onFeedback} /><div className="pulseai-glass rounded-2xl p-5 sm:p-6"><SectionLabel icon={<ShieldCheck size={14} />} label="Workflow boundary" /><h2 className="mt-3 text-base font-semibold text-slate-100">Analysis status</h2><div className="mt-4 rounded-xl border border-amber-400/15 bg-amber-400/[.05] p-4 text-xs leading-relaxed text-amber-100/75"><AlertCircle className="mr-2 inline text-amber-300" size={14} />This demo accepts a local file and shows workflow states only. It does not read, interpret, or medically analyze report contents.</div><div className="mt-5 space-y-3 text-xs text-slate-500"><div className="flex items-center gap-2"><Check size={14} className="text-teal-300" />File type and size checked locally</div><div className="flex items-center gap-2"><Check size={14} className="text-teal-300" />Demo upload state displayed</div><div className="flex items-center gap-2"><X size={14} className="text-slate-600" />Clinical AI analysis not connected</div></div></div></div></>;
}
